...
int s21_sprintf(char *str, const char *format, ...) {  // мы находим константу и мы не можем по ней двигаться в функции
    ...
    while (*format) {
        ...
        // Part 2 - поэтому мы возвращаем стак который мы нашли в строке и присваем ей значения в зависимости
        // от смысла данного значения

        format = get_flags(format, &f);  // функция которая разбирает флаги
        format = get_width(format, &f, va);  // функция которая разбирает ширину
        format = get_precision(format, &f, va);  // функция которая разбирает точность
        format = get_length(format, &f);  // функция которая разбирает длину

        // все эти функции всегда вызываются в не зависимости есть ли они в действительности в строке

        f.specifier = *format;  // записываем в спецификатор этой строки
        format++;

        char buff[BUFF_SIZE] = {'\0'};  // сразу зааполняем изначально весь буфер нулями
        handle_value(f, buff, va);  // в зависимости от спецификатора вызывает функцию парсера
...
void parse_int(flags f, char *buff, va_list va) {
    ...
    switch (f.length) {  // если нет 0 и h то работаем со значение long long которое в себя включает и обычный инт и short
        case 0:
            val = (int32_t)val;  // если высталена ширина 0 - значит работаем с обычным интом
            break;
        case 'h':  // если выставлена h значит работаем с размером short
            val = (int16_t)val;
    }
    whole_num_to_string(val, buff, 10);  // перевод целого числа в строку (функция наже)
...
void whole_num_to_string(int64_t val, char *ret, int base) {
    ...
    int idx = BUFF_SIZE - 2;  // отсекаем минус 2 что-бы оставить \0 в массиве для проверки в будущем по индексу

    bool neg = val < 0 ? 1 : 0;
    val = neg ? -val : val;

    if (val == 0)
        tmp[idx] = '0';  // если значение равно 0 в буфер заносим как 0 потому что в цикл оно не попадает

    while (val > 0) {  // сюда в цикл 0 не попадет из-за проверки
        idx--;  // мы пошли другой дорогой обычно целочисленным делением мы отсекаем конец % 10 и в буфер будет
        // запись идти с конца числа - потом потребуется их переварачивать в буфере чтобы соблюдать последовательность
        tmp[idx] = "0123456789abcdef"[val % base];  // можно делить на 10 но для бонуса делим на base в нем заложено
