 ...
 * Тип данных объектов, на которые указывают как srcptr так и destptr  не имеют
 * никакого значения. Так как эта функция работает с бинарными данными.
 * Функция не проверяет, есть ли символ завершения в srcptr, она всегда копирует
 * количество байтов, указанное в num. Чтобы избежать переполнения блока памяти destptr,
 * размер destptr должен быть не менее num байтов. Однако, может возникнуть ситуация,
 * когда destptr и srcptr пересекутся.
 *
 * Параметры:
